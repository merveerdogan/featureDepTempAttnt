...
// Wrapper for the fullscreen button in the lostFullscreen dialog
function on_check_fullscreen_click() {
    try {
    ...
    // Log a lightweight event to jsPsych if available
    try {
        if (typeof jsPsych !== 'undefined' && jsPsych.data) {
            jsPsych.data.write({ event: 'return_fullscreen_clicked', timestamp: new Date().toISOString() });
        }
    } catch (e) {
        // ignore
    }
...
// Handler used by jsPsych's on_interaction_data_update when referenced as `check_fullscreen`
function check_fullscreen(data) {
    try {
        // Attach viewport size to interaction data when possible
        try {
            var vp = get_viewport_size();
            if (data) {
                data.screen_width = vp.width;
                data.screen_height = vp.height;
            }
        } catch (e) {
            // ignore viewport errors
        }

        // Only act if fullscreen was activated for the experiment and experiment not yet complete
        if (typeof fullscreenActive !== 'undefined' && fullscreenActive && (typeof exp_complete === 'undefined' || exp_complete === false)) {
            var s = JSON.stringify(data || {});
            if (s.includes('blur') || s.includes('fullscreenexit')) {
                // Mark in the interaction data
                if (data) data.fullscreen_exited = true;

                // Prefer the existing dialog `#lostFullscreen` if present
                var lost = document.getElementById('lostFullscreen');
                if (lost) {
                    try {
                        if (typeof lost.showModal === 'function') {
                            lost.showModal();
                        } else {
                ...
                // If they returned to fullscreen, close/hide the dialog
                var lost = document.getElementById('lostFullscreen');
                if (lost) {
                    try {
                        if (typeof lost.close === 'function' && lost.open) {
                            lost.close();
                        } else {
        ...
        console.warn('check_fullscreen error', e);
    }
}
...
function generateInstructions(instructions, enable_continue, button_text, trialCategory = 'instructions') {
    ...
    return {
        ...
        choices: [button_text],
        button_html: '<button class="jspsych-btn" style="padding: 15px 40px; font-size: 18px; font-weight: 400; min-width: 100px;">%choice%</button>',
        enable_button_after: calculatedDelay,
        post_trial_gap: 400,
        data: { trial_category: trialCategory },
    }
}
...
function standard_instr_style(x) {
    ...
    if (Array.isArray(x)) {
        return x.map(function (text) {
            return "<div class='jspsych-display-element' style='justify-content: center; align-items: center; min-height: 80vh; width: 100%;'><div style='display: inline-block; margin: 0 auto; color: " + text_color + "; font-size: 2.5vh; line-height: 3.2vh; padding: 10px 200px 10px 200px; text-align: left'>" + text + "</div></div>";
        });
    } else {
        return ["<div class='jspsych-display-element' style='justify-content: center; align-items: center; min-height: 80vh; width: 100%;'><div style='display: inline-block; margin: 0 auto; color: " + text_color + "; font-size: 2.5vh; line-height: 3.2vh; padding: 10px 200px 10px 200px; text-align: left'>" + x + "</div></div>"];
    }
}

function practice_instr_style(x) {
    return ["<div class='jspsych-display-element' style='justify-content: flex-start; align-items: center; min-height: 10vh; width: 100%; padding-top: 5vh;'><div style='display: inline-block; margin: 0 auto; color: " + text_color + "; font-size: 2.5vh; line-height: 3.2vh; padding: 10px 200px 10px 200px; text-align: left'>" + x + "</div></div>"];
}
...
var cursor_off = {
    ...
    data: { trial_category: 'cursor_off' },
}
...
var finishing = generateInstructions(finish, false, 'Next', 'closingInstructions');
...
var debreif_qs = {
    ...
    data: { trial_category: 'demographicQuestions' },
    on_start: function () {
    ...
    on_finish: function (data) {
        ...
        // saveData('filtered_' + participantID, test_data.csv(), save_folder_filtered);
...
function debriefForm() {
    var debriefContent = '<p>Finally, we just have a couple questions for you!<br>Please note that you must answer <strong>ALL</strong> the questions before clicking "Continue". ' +
        ...
        '<p>Age: <br><input name="age" required type="number" max="100" min="18" style="width: 80px; border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 15px" required></p>' +
        '<p>Please select your gender:<br><input type="radio" required id="male" name="gender" value="male"><label for="male">Male</label><br><input type="radio" id="female" name="gender" value="female"><label for="female">Female</label><br><input type="radio" id="other" name="gender" value="other"><label for="other">Other</label><br><input type="radio" id="not_say" name="gender" value="Prefer not to say"><label for="not_say">Prefer not to say</label></p>' +
        '<p>In 1-2 sentences, what do you think this experiment was testing? <br><input name="testing" type="text" size="50" style="width: 100%;  border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 15px" required></p>' +
        '<p>Using the slider below, on a scale of 1-100 (with 1 being very distracted, and 100 being very focused), how well did you pay attention to the experiment?  (This will not affect whether you receive credit or compensation.) <br> <div style="text-align:left;">Very Distracted <span style="float:right;">Very Focused</span></div> <input type="range" value="50" min="1" max="100" name="attention" class="slider" style="width: 100%; margin: 8px 0;" required oninput="this.nextElementSibling.value = this.value"> <output id="attntion_out" style="text-align:center; font-size: 1.6vw">_</output> </p>' + '<p>Did you find yourself using a strategy while you were doing the experiment? If yes, please describe with 1-2 sentences.<br><input name="strategy" type="text" size="50" style="width: 100%; border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 15px" required></p>' + '<p>Is there anything else we should know (either about you or how you did the experiment) that may have had an impact on your results? <br><input name="other" type="text" size="50" style="width: 100%; border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 15px" required></p>' +
...
function closingText() {
    ...
    var addition = "<p>Here is your <b>unique</b> code: " + completion_code + "</p><p>To recieve payment for this experiment, you must take this code back to the Prolific page that directed you here. You can finish the experiment by clicking the 'Finish' button below. If you encounter problems during this step, please contact the email above.";
...
function consentForm() {
    // If a Qualtrics link is provided in params.js (`qualtricsConsentURL`), redirect/open it and do not show inline consent text.
    var qURL = (typeof qualtricsConsentURL !== 'undefined' && qualtricsConsentURL) ? qualtricsConsentURL : null;
    if (qURL) {
        var content = "<div style='color: " + text_color + "; text-align:center; padding: 30px;'>" +
            "<p style='font-size:18px; font-weight:700;'>You will be taken to the consent form now.</p>" +
            "<p>Please complete the consent form in the new tab. After finishing, return to this page and press the 'Y' key to continue.</p>" +
            "<p>If the consent form did not open automatically, <a href='" + qURL + "' target='_blank' rel='noopener'>click here to open the consent form</a>.</p>" +
            "</div>";
    ...
    // No Qualtrics link: show the full inline consent text as before
    var inlineConsent = "<div style='display: inline-block; color: " + text_color + "; margin: 0 auto; padding: 10px 80px 10px 80px; text-align: left'><h1>Consent for Participation in a Research Study</h1>" +
        "<h5>STUDY TITLE</h5><p>" + study_title + "</p>" +
        ...
        "<h5>DURATION</h5><p>If you agree to take part, the study will last approximately <strong> " + String(estTotalRunTime) + " minutes</strong>.</p>" +
        "<h5>COSTS AND COMPENSATION</h5><p>There are no costs associated with participation in this study. You will receive <strong>" + String(exp_compensation) + "</strong> for participating.</p>" +
        ...
        "<h5>LEARNING MORE</h5><p>If you have questions about this study, you may contact your experimenter Merve Erdogan at merve.erdogan@yale.edu. If you have questions about your rights as a research participant, or you have complaints about this research, you can contact the Yale Institutional Review Boards at 203-785-4688 or hrpp@yale.edu.</p>" +
        "<h5>INFORMED CONSENT</h5><p>Your participation indicates that you have read and understood this consent form and the information presented and that you agree to be in this study.</p></div>";

    var content = "<div style='color: " + text_color + ";'>" +
        "<div style='text-align:center; font-weight:bold; margin-bottom:12px;'>Please read the consent information below.</div>" +
        inlineConsent +
        "</div>";
